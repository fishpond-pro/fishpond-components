...
const addFilters = (url, filters) => {
    ...
    const newUrl = url.replace(new RegExp(`(/${keys.join('/\\w+|/')}/\\w+)`, 'g'), '');
...
const ProcessItems = async (ctx) => {
    ...
    const items = works.toArray().map((item) => {
        ...
        let images = item.find('div[data-samples]').length === 0 ? [] : JSON.parse(item.find('div[data-samples]').attr('data-samples').replace(/'/g, '"')).map((s) => s.thumb);
